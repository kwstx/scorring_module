...
export interface ResourceAnalysis {
    ...
    opportunityTradeoffScore: number;
...
export class ResourceAnalyzer {
    public analyze(
        ...
        requiredResources: DecisionObject['requiredResources']
    ): ResourceAnalysis {
        ...
        const projectedOpportunityCostOfBlockingUSD = this.computeOpportunityCostOfBlockingUSD(
            action,
            estimatedFinancialExpenditureUSD,
            opportunityTradeoffScore
        );
        ...
        const economicEfficiencyScore = this.clamp01(
            (normalizedOpportunity * 0.7) + ((1 - normalizedSpend) * 0.3)
        );
    ...
    private computeComputationalCost(requiredResources: DecisionObject['requiredResources']): number {
        let cpuMillis = 0;
        let ioWeight = 0;
    ...
    private computeFinancialExpenditure(requiredResources: DecisionObject['requiredResources']): number {
    ...
    private computeBandwidthUtilization(
        action: RawAgentAction,
        requiredResources: DecisionObject['requiredResources']
    ): number {
    ...
    private computeOpportunityTradeoff(
        action: RawAgentAction,
        requiredResources: DecisionObject['requiredResources']
    ): number {
        ...
        const costlyResources = requiredResources.reduce((sum, resource) => {
            const criticalityWeight =
                resource.criticality === 'HIGH' ? 1 : resource.criticality === 'MEDIUM' ? 0.6 : 0.3;
    ...
    private computeOpportunityCostOfBlockingUSD(
        action: RawAgentAction,
        estimatedFinancialExpenditureUSD: number,
        opportunityTradeoffScore: number
    ): number {
        ...
        const classMultiplier =
            actionLabel.includes('DELETE') ? 0.7 :
