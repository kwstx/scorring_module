...
export class StrategicAlignmentModule {
    ...
    evaluate(decision) {
        ...
        const overallAlignmentScore = this.computeOverallAlignment(goalAlignments, initiativeAlignments, cooperativeImpactAlignments);
        // 5. Derive the misalignment penalty for the risk engine
        const misalignmentPenalty = this.computeMisalignmentPenalty(overallAlignmentScore, goalAlignments, initiativeAlignments, cooperativeImpactAlignments);
        // 6. Collect human-readable alignment flags
        const alignmentFlags = this.collectAlignmentFlags(goalAlignments, initiativeAlignments, cooperativeImpactAlignments);
        return {
    ...
    evaluateGoalAlignment(actionText, decision) {
        return this.goals.map((goal) => {
            ...
            const domainOverlap = goal.domains.some((domain) => actionText.includes(domain.toLowerCase()))
                ? 0.15
                : 0;
            // Base alignment signal: positive from alignment hits, negative from misalignment hits
            const alignmentSignal = matchedAlignment.length * 0.25 - matchedMisalignment.length * 0.35;
            // Horizon multiplier — misalignment with long-term goals is heavier
            const horizonMultiplier = goal.horizon === 'LONG_TERM' ? 1.3 : goal.horizon === 'MEDIUM_TERM' ? 1.0 : 0.8;
            ...
            const contribution = alignmentScore >= 0.5
                ? 'STRONGLY_ALIGNED'
                : alignmentScore >= 0.15
                    ? 'ALIGNED'
                    : alignmentScore > -0.15
    ...
    // ------------------------------------------------------------------
    evaluateInitiativeAlignment(actionText, decision) {
        return this.initiatives.map((initiative) => {
            ...
            const resourceConflict = initiative.resourceDomains.some((domain) => actionResources.includes(domain.toLowerCase()));
            // Status interference: winding-down initiatives should not receive new resource load
            const statusInterference = initiative.status === 'WINDING_DOWN' &&
                matchedSynergy.length === 0 &&
                resourceConflict;
            // Synergy score
            let synergySignal = matchedSynergy.length * 0.3 - matchedConflict.length * 0.4;
            ...
            const synergyScore = Number(this.clamp(synergySignal * initiative.importanceWeight, -1, 1).toFixed(4));
            return {
    ...
    // ------------------------------------------------------------------
    evaluateCooperativeImpactAlignment(actionText, decision) {
        return this.cooperativeObjectives.map((objective) => {
            ...
            let impactSignal = matchedPositive.length * 0.3 - matchedErosion.length * 0.4;
            ...
            const impactScore = Number(this.clamp(impactSignal * objective.weight, -1, 1).toFixed(4));
            return {
     ...
     *   Cooperative:    25%
     */
    computeOverallAlignment(goalAlignments, initiativeAlignments, cooperativeAlignments) {
        const goalAvg = this.weightedAverage(goalAlignments.map((g) => g.alignmentScore), this.goals.map((g) => g.priority));
        const initiativeAvg = this.weightedAverage(initiativeAlignments.map((i) => i.synergyScore), this.initiatives.map((i) => i.importanceWeight));
        const cooperativeAvg = this.weightedAverage(cooperativeAlignments.map((c) => c.impactScore), this.cooperativeObjectives.map((c) => c.weight));
        ...
        return this.clamp01(normalizedGoal * 0.4 + normalizedInitiative * 0.35 + normalizedCooperative * 0.25);
    }
    /**
     ...
     */
    computeMisalignmentPenalty(overallAlignment, goalAlignments, initiativeAlignments, cooperativeAlignments) {
        ...
        const misalignedGoals = goalAlignments.filter((g) => g.contribution === 'MISALIGNED' || g.contribution === 'STRONGLY_MISALIGNED');
        const conflictedInitiatives = initiativeAlignments.filter((i) => i.synergyScore < -0.1 || i.resourceConflict || i.statusInterference);
        ...
        if (misalignedGoals.length > 0) {
            const goalPenaltyBoost = misalignedGoals.reduce((sum, g) => sum + Math.abs(g.alignmentScore), 0) /
                misalignedGoals.length;
            penalty += goalPenaltyBoost * 0.15;
        }
        if (conflictedInitiatives.length > 0) {
            const initiativePenaltyBoost = conflictedInitiatives.reduce((sum, i) => sum + Math.abs(i.synergyScore) + (i.statusInterference ? 0.1 : 0), 0) / conflictedInitiatives.length;
            penalty += initiativePenaltyBoost * 0.12;
        }
        if (erodedObjectives.length > 0) {
            const cooperativePenaltyBoost = erodedObjectives.reduce((sum, c) => sum + Math.abs(c.impactScore), 0) /
        ...
        const misalignmentBreadth = (misalignedGoals.length > 0 ? 1 : 0) +
    ...
    // Alignment Flags
    // ------------------------------------------------------------------
    collectAlignmentFlags(goalAlignments, initiativeAlignments, cooperativeAlignments) {
        const flags = [];
        for (const goal of goalAlignments) {
            if (goal.contribution === 'STRONGLY_MISALIGNED') {
                flags.push(`CRITICAL: Action strongly misaligned with goal "${goal.goalTitle}" (score: ${goal.alignmentScore})`);
            }
            else if (goal.contribution === 'MISALIGNED') {
                flags.push(`WARNING: Action misaligned with goal "${goal.goalTitle}" (score: ${goal.alignmentScore})`);
            }
        }
        for (const init of initiativeAlignments) {
            if (init.statusInterference) {
                flags.push(`WARNING: Action interferes with winding-down initiative "${init.initiativeName}"`);
            }
            if (init.resourceConflict && init.synergyScore < 0) {
                flags.push(`CAUTION: Resource conflict detected with initiative "${init.initiativeName}"`);
            }
            if (init.matchedConflictIndicators.length > 0) {
                flags.push(`WARNING: Action conflicts with initiative "${init.initiativeName}" — matched: [${init.matchedConflictIndicators.join(', ')}]`);
            }
        }
        for (const coop of cooperativeAlignments) {
            if (coop.impactScore < -0.3) {
                flags.push(`CRITICAL: Action erodes cooperative objective "${coop.objectiveTitle}" (score: ${coop.impactScore})`);
            }
            else if (coop.impactScore < -0.1) {
                flags.push(`WARNING: Action may undermine cooperative objective "${coop.objectiveTitle}" (score: ${coop.impactScore})`);
            }
        }
        return flags;
    }
     ...
     */
    buildActionTextCorpus(decision) {
        const parts = [
            decision.actionType,
            decision.intent,
     ...
     */
    matchIndicators(actionText, indicators) {
        return indicators.filter((indicator) => actionText.includes(indicator.toLowerCase()));
    }
    /**
     ...
     */
    loadDefaultStrategicContext() {
        // ---- Organizational Goals ----
        this.goals = [
            {
                id: 'GOAL-001',
                title: 'Strengthen Data Governance',
                description: 'Ensure all data handling follows strict governance, access controls, and audit requirements.',
                priority: 0.95,
                alignmentIndicators: [
            ...
            },
            {
                ...
                description: 'Drive migration from legacy systems to modern, cloud-native architectures.',
                priority: 0.85,
                alignmentIndicators: [
            ...
            },
            {
                ...
                description: 'Build systems that recover quickly from failure and maintain service under stress.',
                priority: 0.9,
                alignmentIndicators: [
            ...
            },
            {
                ...
                description: 'Systematically reduce unnecessary operational expenditure while maintaining quality.',
                priority: 0.7,
                alignmentIndicators: [
        ...
        this.initiatives = [
            {
                ...
                description: 'Implementing zero-trust network architecture across all services with continuous identity verification.',
            ...
            },
            {
                ...
                description: 'Final phase of migrating core services to cloud-native infrastructure.',
            ...
            },
            {
                ...
                description: 'Upgrading audit trail systems to provide immutable, queryable records of all system operations.',
        ...
        this.cooperativeObjectives = [
            {
                ...
                description: 'Maintain and strengthen trust across partner ecosystems through transparent, predictable behavior.',
            ...
            },
            {
                ...
                description: 'Contribute to the resilience of shared infrastructure and common platforms used by the broader ecosystem.',
            ...
            },
            {
                ...
                description: 'Foster conditions for continuous improvement in cooperative decision-making across agents and organizations.',
